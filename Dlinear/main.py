...
class SCINetinitialization():
    ...
    def predict(self, setting, load=False):
        ...
        for target in ['avgmem', 'avgcpu']:
            ...
            if args.rollingforecast:
                df = pd.DataFrame(
                    {f'{target.capitalize()}real': pre_data['{}'.format(target)][:len(preds)], 'forecast': preds})
                df.to_csv('./results/{}-ForecastResults.csv'.format(target), index=False)
            else:
            ...
            if args.show_results:
                ...
                # 创建折线图
                plt.plot(df[f'{target.capitalize()}real'].tolist(), label='real', color='blue')  # 实际值
                plt.plot(df['forecast'].tolist(), label='forecast', color='red', linestyle='--')  # 预测值

                # 增强视觉效果
                plt.grid(True)
                plt.title(f'{target.upper()} real vs forecast')
                plt.xlabel('date')
                plt.ylabel(f'{target.upper()}')
                plt.legend()
