...
class EncoderLayer(nn.Module):
    def __init__(self, n_heads, d_model, d_ff=None, dropout=0.1, activation="relu", output_attention=False):
        ...
        self.attention = Attention(n_heads, mask_flag=False, attention_dropout=dropout,
...
class DecoderLayer(nn.Module):
    def __init__(self, n_heads, d_model, d_ff=None, dropout=0.1, activation="relu", output_attention=False):
        ...
        d_ff = d_ff or 4 * d_model
        self.self_attention = Attention(n_heads, mask_flag=True, attention_dropout=dropout,
                                        output_attention=output_attention)
        self.cross_attention = Attention(n_heads, mask_flag=False, attention_dropout=dropout,
                                         output_attention=output_attention)
        self.norm_1 = nn.LayerNorm(d_model)
        self.norm_2 = nn.LayerNorm(d_model)
